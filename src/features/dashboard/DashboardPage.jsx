...
const calculateCollectionRate = (collected, due) => {
  if (due === 0) return 0;
...
const GlobalAdjustmentForm = ({ onAdjustmentAdded }) => {
...
// New component for Global Adjustment Tabs with more subtle edit buttons
const GlobalAdjustmentTabs = ({ globalAdjustments, onAdjustmentUpdated, notifications }) => {
  const activeTab = "all";
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      setIsSubmitting(true);
      await updateSetting(editingAdjustment.id, {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Adjustments grid with more subtle edit buttons */}
              ...
              <form onSubmit={handleUpdate} className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    from {formatDate(adj.effectiveFrom)}
                  </span>
                </div>
                <div>
...
const SubordinateCard = ({ metrics, onClick, isExpanded }) => {
  return (
    <div
...
export default function DashboardPage() {
  ...
  const [expandedSubordinates, setExpandedSubordinates] = useState(new Set());
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const responseData = await fetchHierarchy();
  ...
  const processSubordinateMetrics = (data) => {
    const allMetrics = [];
  ...
  const getMetrics = () => {
    ...
    const processHierarchy = (subordinates) => {
  ...
  return (
    <DashboardLayout>
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {/* Tax and Other expenses - Updated section */}
        <div className="mb-8">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                Across {metrics.totalSubordinates} subdivisions
              </div>
            </div>
